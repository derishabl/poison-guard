...
def test_qrels_validate_smoke():
    with tempfile.TemporaryDirectory() as d:
        ...
        qrels = {"q1": {"C": 1}, "q2": {"A": 1}}
        pp, qp, qq, out = (os.path.join(d, n) for n in
                           ("probe.json", "qrels.json", "queries.jsonl", "out.json"))
        json.dump(probe, open(pp, "w"))
        json.dump(qrels, open(qp, "w"))
        with open(qq, "w") as f:
            f.write('{"id": "q1"}\n{"id": "q2"}\n')

        r = subprocess.run([sys.executable, SCRIPT, "--probe", pp, "--qrels", qp,
                            "--queries", qq, "--json", out],
...
def test_qrels_validate_mismatched_queries_errors():
    with tempfile.TemporaryDirectory() as d:
        ...
        with open(qq, "w") as f:
            f.write('{"id": "q1"}\n{"id": "q2"}\n')  # 2 запроса vs 1 hits
        r = subprocess.run([sys.executable, SCRIPT, "--probe", pp, "--qrels", qp,
                            "--queries", qq], capture_output=True, text=True)
