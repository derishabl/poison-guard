...
class FaissAdapter(BaseVectorStoreAdapter):
    ...
    def __init__(self, index_path: str, ids_map_path: str | None = None):
        ...
        if len(self._ids) != self._index.ntotal:
            raise ValueError(
                f"ids-map length {len(self._ids)} != index ntotal {self._index.ntotal}"
            )
...
def build_flat_index(vectors: list[list[float]], ids: list[str],
                     index_path: str, ids_map_path: str,
                     metric: str = "ip") -> None:
    """
