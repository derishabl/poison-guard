"""
synth.py — синтетические запросы из корпуса (для команд без query-логов).

Exposure зависит от распределения реальных запросов; синтетика — приближение.
Но для команд без логов это единственный способ запустить аудит.
    ...
    for i, c in enumerate(chunks):
        for j in range(n_per_chunk):
            if query_style == "text":
                ...
                q_text = _keyword_query(c.text, vec, n_terms=n_terms)
...
def synth_probe(
    ...
    return probe(store, queries, top_k=top_k, corpus_ids=[c.id for c in chunks])
